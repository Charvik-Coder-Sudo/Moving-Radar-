...
if [ -z "$PY_CMD" ]; then
 ...
 FAILED: no Python 3.10 or newer was found.
   ...
   fix:  Debian/Ubuntu : sudo apt install python3 python3-venv python3-pip
...
if [ -x "$PROJECT_ROOT/.venv/bin/python" ]; then
 ...
 FAILED: could not create the virtual environment.
   ...
   why:  the venv module is often packaged separately on Debian/Ubuntu.
   fix:  sudo apt install python3-venv
         (or check that you can write to $PROJECT_ROOT)
EOF
        exit 1
    fi
fi
...
if ! "$VENV_PY" -m pip install -r "$PROJECT_ROOT/requirements.txt" --disable-pip-version-check; then
 ...
 FAILED: the dependencies could not be installed.
   ...
   fix:  check the connection and run this script again. For a company index:
         PIP_INDEX_URL=https://your/index ./setup_linux.sh
EOF
    exit 1
fi
...
if [ $RC -ne 0 ]; then
    ...
    echo " setup complete."
    echo
    echo "   Start the display with:   ./run_linux.sh"
    echo "   Scenario Export folder:   set it in config/display_config.json, or"
    echo "                             ./run_linux.sh --export /path/to/Scenario_Export"
fi
echo
exit $RC
